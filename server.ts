...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'tube-dist/begrateful-tube/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(
        join(process.cwd(), 'tube-dist/begrateful-tube/browser'),
        'index.html'
      )
    )
    .toString();
  ...
  server.get('*', (req, res) => {
    res.render(
      ...
      async (err, html) => {
        ...
        var seo: any = {
          title: 'BeGrateful.tube',
          description:
            'The Umbrella platform for All freedom based projects worldwide',
          image: 'https://begrateful.tube/assets/begratefultube-logo.png',
          site: 'https://begrateful.tube/',
